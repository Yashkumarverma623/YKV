...
export default function SkillsSection() {
  ...
  const skills = [
    ...
    { name: "GSAP", type: "framework", icon: "https://cdn.jsdelivr.net/npm/simple-icons@v11/icons/greensock.svg" },
    // Backend
    ...
    { name: "JWT Auth", type: "backend", icon: "" },
    ...
    { name: "PostgreSQL", type: "backend", icon: "https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postgresql/postgresql-original.svg" },
    // AI / ML
    ...
    { name: "Scikit-learn", type: "ai", icon: "https://cdn.jsdelivr.net/npm/simple-icons@v11/icons/scikitlearn.svg" },
    { name: "Hugging Face", type: "ai", icon: "https://cdn.jsdelivr.net/npm/simple-icons@v11/icons/huggingface.svg" },
    { name: "Pandas", type: "ai", icon: "https://cdn.jsdelivr.net/gh/devicons/devicon/icons/pandas/pandas-original.svg" },
    { name: "NumPy", type: "ai", icon: "https://cdn.jsdelivr.net/gh/devicons/devicon/icons/numpy/numpy-original.svg" },
    // Vector DBs & RAG
    { name: "ChromaDB", type: "data", icon: "https://cdn.jsdelivr.net/npm/simple-icons@v11/icons/chroma.svg" },
    { name: "Pinecone", type: "data", icon: "https://cdn.jsdelivr.net/npm/simple-icons@v11/icons/pinecone.svg" },
    { name: "Qdrant", type: "data", icon: "https://cdn.jsdelivr.net/npm/simple-icons@v11/icons/qdrant.svg" },
    { name: "RAG", type: "data", icon: "" },
    // DevOps & Tools
    ...
    { name: "AWS", type: "devops", icon: "https://cdn.jsdelivr.net/gh/devicons/devicon/icons/amazonwebservices/amazonwebservices-original-wordmark.svg" },
    ...
    { name: "Postman", type: "devops", icon: "https://cdn.jsdelivr.net/npm/simple-icons@v11/icons/postman.svg" },
      ...
      <section id="skills" className="relative w-full h-[85vh] bg-[#070708] text-white overflow-hidden z-[20] bg-[radial-gradient(ellipse_80%_80%_at_50%_-20%,rgba(227,247,148,0.06),rgba(0,229,255,0.02),rgba(255,255,255,0))] bg-[linear-gradient(rgba(255,255,255,0.01)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.01)_1px,transparent_1px)] bg-[size:40px_40px]">
        ...
        <div ref={containerRef} className="absolute inset-0 cursor-crosshair z-10">
          {skills.map((skill, index) => (
            <div
              key={index}
              className={`matter-object absolute w-max font-tech font-semibold rounded-full cursor-grab select-none pointer-events-auto z-10 active:cursor-grabbing flex items-center gap-2.5
                ${skill.type === 'language' 
                  ? 'bg-gold text-black border border-gold/20 shadow-[0_0_15px_rgba(255,215,0,0.15)] text-lg md:text-2xl px-6 py-3 md:px-7 md:py-3.5' 
                  : skill.type === 'ai' 
                    ? 'bg-cyan text-black border border-cyan/20 shadow-[0_0_15px_rgba(0,229,255,0.2)] text-base md:text-lg px-5 py-2.5' 
                    : 'bg-[#121214]/60 backdrop-blur-md border border-white/5 text-white/95 hover:border-white/15 text-sm md:text-base px-4 py-2 transition-colors duration-200'}`}
            >
              {skill.icon && (
                <img 
                  src={skill.icon} 
                  alt={skill.name}
                  className={`object-contain transition-all ${skill.type === 'language' ? 'w-6 h-6 md:w-8 md:h-8' : 'w-5 h-5 md:w-6 md:h-6 opacity-80 group-hover:opacity-100'}`}
        ...
        <div className="absolute top-0 left-0 w-full h-full flex flex-col justify-center items-center p-8 pointer-events-none z-[2]">
          ...
          <p className="font-tech text-xs text-white/30 tracking-[0.2em] uppercase mt-4 pointer-events-auto">
            Grab and toss the capsules to break gravity
          </p>
        </div>
      </section>
    </div>
